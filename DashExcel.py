...
df['intake_time'] = pd.to_datetime(df['intake_time'])  # convert
df['intake_time'] = df['intake_time'].dt.hour  # only extra hour
# print(df.head())
...
app.layout = html.Div([
    html.H1('Analytics Dashboard of Dallas Animal Shelter (Dash Plotly', style={'textAlight': 'center'}),
    html.Hr(),
    html.P('Choose animal of internet: '),
    html.Div(html.Div([
        dcc.Dropdown(id='animal-type', clearable=False,
                     value='DOG',
                     options=[{
                         'label': x,
                         'value': x} for x in df['animal_type'].unique()
                     ]),

    ], className='two columns'), className='row'),  # end div 1
...
@app.callback(Output(component_id='output-div', component_property='children'),
              ...
              )  # the # of the input value = # of parameter in def function
# component_property='value' = def make_graphs(animal_chosen)
# all the below graphs will go into html.Div(id='output-div', children=[]),
...
def make_graphs(animal_chosen):
    ...
    # LINE CHART
    df_line = df.sort_values(by=["intake_time"], ascending=True) # go from small # to larger # by order
